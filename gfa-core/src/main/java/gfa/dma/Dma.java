...
public class Dma
{
  ...
  protected void blit()
  {
    ...
    //System.out.println("unsignedCount = " + unsignedCount);
    //System.out.println("toAddToSrc = " + toAddToSrc);
    //System.out.println("toAddToDst = " + toAddToDst);
    
    // Process the copy
    if ((cr & DMAXCrWidthBit) == 0) // if the copy is 16bits-based
      for (int i = 0; i < unsignedCount; i++)
      {
        try {mem.storeHalfWord(dst, mem.loadHalfWord(src));}
      ...
      for (int i = 0; i < unsignedCount; i++)
      {
        try {mem.storeWord(dst, mem.loadWord(src));}
	catch(AbortException e){}
        src += toAddToSrc;
        dst += toAddToDst;
      }
    ...
    if (irqEnabled)
      ioMem.genInterrupt(interruptBit); // generated an interrupt.
  }
